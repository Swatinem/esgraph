...
function ControlFlowGraph(astNode) {
	...
	function createNodes(astNode) {
		walker(astNode, { default: function () {
			...
			// do not recurse for FunctionDeclaration, ExpressionStatement or any sub-expression
			if (~['FunctionDeclaration', 'ExpressionStatement'].indexOf(this.type) ||
			    ~this.type.indexOf('Expression'))
				return;
